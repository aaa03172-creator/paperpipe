...
from src.services.evidence_grounding_benchmark import build_evidence_grounding_benchmark_manifest_from_staged_gold


def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Build a fixed paper-understanding goldset manifest from staged review gold, "
            "then build an evidence-grounding benchmark manifest from that fixed target."
        )
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Allow schema-valid but not eval-ready gold records in the generated manifests.",
    )
    parser.add_argument(
        "--allow-missing-runs",
        action="store_true",
    ...
    print(f"[evidence_grounding_benchmark_manifest_from_staged_gold] benchmark_id={manifest.benchmark_id}")
    print(f"[evidence_grounding_benchmark_manifest_from_staged_gold] item_count={len(manifest.items)}")
    print(
        "[evidence_grounding_benchmark_manifest_from_staged_gold] "
        f"goldset_manifest={Path(args.goldset_manifest_out).expanduser().resolve()}"
    )
    print(f"[evidence_grounding_benchmark_manifest_from_staged_gold] out={Path(args.out).expanduser().resolve()}")
    return 0
