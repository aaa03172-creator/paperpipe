...
from src.schemas.bc5cdr_eval import BC5CDRDocument  # noqa: E402
from src.services.bc5cdr_adapter import project_bc5cdr_to_evidence_extraction_bundle  # noqa: E402
from src.services.bc5cdr_eval import evaluate_bc5cdr_bundle, load_bc5cdr_document, write_bc5cdr_eval_report  # noqa: E402
from src.services.bc5cdr_prediction import (  # noqa: E402
    build_bc5cdr_prediction_inputs,
    build_bc5cdr_prediction_prompt,
    load_manifest,
    repair_bc5cdr_prediction_payload,
...
def generate_predictions(
    *,
    ...
    for doc in documents:
        ...
        paper = build_bc5cdr_prediction_inputs(source_path, paper_id=effective_paper_id)
        prompt = build_bc5cdr_prediction_prompt(paper=paper, schema_json=schema_json)
        ...
        try:
            ...
            prediction_document = BC5CDRDocument.model_validate(repaired)
            ...
            bundle = project_bc5cdr_to_evidence_extraction_bundle(
                ...
                run_id=run_id,
                source_artifact="bc5cdr_prediction.json",
            )
            ...
            report_file = write_bc5cdr_eval_report(report, eval_root / f"{safe_id}.json")
    ...
    _write_json(
        ...
        {
            "schema_version": "bc5cdr_prediction_manifest.v1",
    ...
    metrics = {
        "schema_version": "bc5cdr_prediction_generation.v1",
        "generated_at": _utc_now_iso(),
        "run_id": run_id,
        "source_manifest": str(manifest_path),
        "model": model,
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate BC5CDR-style predictions from document artifacts, convert them into evidence bundles, and score them.",
    )
    parser.add_argument("--manifest", required=True, help="Manifest with BC5CDR gold_path plus gold_source_paths document artifacts.")
    parser.add_argument("--out-dir", default="snapshots/bc5cdr_prediction_generation", help="Output root directory.")
...
def main() -> None:
    ...
    run_id = args.run_id or datetime.now(timezone.utc).strftime("bc5cdr_prediction_generation_%Y%m%d_%H%M%S")
    ...
    print(f"[generate_bc5cdr_bundle_predictions] out={run_root}")
    print(f"[generate_bc5cdr_bundle_predictions] metrics={run_root / 'metrics.json'}")
