...
def _reviewer_value_hint_counts_by_field(
    *,
    ...
    for field_name, count in missing_value_counts.items():
        missing_suggestions = missing_suggestion_counts
        hint_count = count - missing_suggestions.get(field_name, 0)
        if hint_count > 0:
...
def _print_threshold_package_diagnostics(report, *, prefix: str) -> None:
    for evidence_key, output_key in (
        ...
        ("baseline_scorecard_not_ready_by_split", "threshold_package_baseline_scorecard_not_ready_by_split"),
        ("candidate_scorecard_not_ready_by_split", "threshold_package_candidate_scorecard_not_ready_by_split"),
        ...
        ("baseline_scorecard_fail_repair_targets", "threshold_package_baseline_scorecard_fail_repair_targets"),
        ("candidate_scorecard_fail_repair_targets", "threshold_package_candidate_scorecard_fail_repair_targets"),
        ("baseline_scorecard_not_ready_repair_targets", "threshold_package_baseline_scorecard_repair_targets"),
        ("candidate_scorecard_not_ready_repair_targets", "threshold_package_candidate_scorecard_repair_targets"),
    ):
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Build contract readiness and final roadmap completion audit artifacts from a "
            "threshold-adoption package. All outputs remain non-canonical review artifacts."
        )
    )
    ...
    parser.add_argument("--representative-split")
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Additional review/contract artifact to include in generated contract compatibility, for example "
            "claim_evidence_correction_repair_plan.json, p0_overstatement_review_packet.json, or "
    ...
    print(f"[evidence_grounding_roadmap_completion_threshold_package] audit_id={report.audit_id}")
    print(f"[evidence_grounding_roadmap_completion_threshold_package] pass_count={report.pass_count}")
    print(f"[evidence_grounding_roadmap_completion_threshold_package] fail_count={report.fail_count}")
    print(f"[evidence_grounding_roadmap_completion_threshold_package] roadmap_complete={report.roadmap_complete}")
    print(
        "[evidence_grounding_roadmap_completion_threshold_package] "
    ...
    print(f"[evidence_grounding_roadmap_completion_threshold_package] next_action_count={report.next_action_count}")
    print(
        "[evidence_grounding_roadmap_completion_threshold_package] "
    ...
    print(f"[evidence_grounding_roadmap_completion_threshold_package] next_action_phase_counts={phase_counts or '-'}")
    print(
        "[evidence_grounding_roadmap_completion_threshold_package] "
    ...
    print(
        "[evidence_grounding_roadmap_completion_threshold_package] "
        f"threshold_adoption_package={_stdout_value(Path(args.threshold_adoption_package).resolve())}"
    )
    print(
        "[evidence_grounding_roadmap_completion_threshold_package] "
    ...
    if args.print_action_queue_counts or args.print_all_action_queues:
        _print_action_queue_counts(report, prefix="evidence_grounding_roadmap_completion_threshold_package")
    if args.print_blocker_summary:
        _print_blocker_summary(report, prefix="evidence_grounding_roadmap_completion_threshold_package")
    if args.print_placeholder_summary:
        _print_placeholder_summary(report, prefix="evidence_grounding_roadmap_completion_threshold_package")
