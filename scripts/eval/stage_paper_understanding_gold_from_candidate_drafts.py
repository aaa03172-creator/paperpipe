...
from src.services.paper_understanding_gold_drafts import stage_paper_understanding_gold_from_candidate_drafts
from src.services.runtime_paths import goldset_root as default_goldset_root


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Stage curated paper-understanding gold JSON files from non-canonical candidate drafts."
    )
    parser.add_argument("drafts", nargs="+", help="paper_understanding_gold_candidate_draft.v1 JSON files.")
    parser.add_argument(
        "--goldset-root",
    ...
    print(f"[paper_understanding_gold_staging] staged_count={manifest.staged_count}")
    print(f"[paper_understanding_gold_staging] out_dir={manifest.out_dir}")
    print(f"[paper_understanding_gold_staging] manifest={out_dir / 'staging_manifest.json'}")
    return 0
