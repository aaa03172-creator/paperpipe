...
def test_template_trial_includes_institutional_block_when_pdf_missing():
    ...
    assert "Clinical Quick Look" in md
...
def test_template_trial_includes_missing_pdf_warning_when_no_proxy_link_exists():
    paper = _base_paper()
    paper["feedback_json"] = "{}"
    ...
    assert "PDF Unavailable" in md
    assert "no institutional access link is stored for this paper yet" in md


def test_template_trial_uses_specialty_lane_for_trial_extraction():
    paper = _base_paper()
    extraction = SpecialtyTrialExtraction(
        paper_id="paper-1",
        citation={
            "title": "Clinical Trial X",
            "authors_first": "Kim",
