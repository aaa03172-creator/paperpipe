...
from src.chart_packs.store import (
    ...
    load_chart_pack_render,
    load_chart_pack_spec,
    save_chart_pack_artifact_json,
    save_chart_pack_bundle,
)
from src.schemas.chart_pack import ChartPack


def _sample_chart_pack(title: str = "Chart pack demo", *, include_render_ref: bool = False) -> ChartPack:
    return ChartPack(
        ...
        charts=[
            {
                ...
                "render_refs": [{"kind": "render_svg", "path": "renders/chart_1.svg"}] if include_render_ref else [],
            }
        ],
    )
...
def test_chart_pack_store_roundtrip_persists_svg_render(tmp_path) -> None:
    root = tmp_path / "chart_packs"
    chart_pack = _sample_chart_pack(include_render_ref=True)
    markdown = "# Chart Pack\n"
    ...
    result = save_chart_pack_bundle(
        ...
        data_snapshots={"chart_1": "status,count\nverified,4\n"},
        specs={"chart_1": {"type": "bar"}},
        renders={"chart_1": {"svg": "<svg xmlns=\"http://www.w3.org/2000/svg\"></svg>"}},
        root=root,
    )

    assert result["render:chart_1:svg"] == chart_pack_render_path(chart_pack.chart_pack_id, "chart_1", root=root)
    assert load_chart_pack_render(chart_pack.chart_pack_id, "chart_1", root=root) == (
...
def test_chart_pack_store_removes_stale_snapshot_and_spec_files_on_overwrite(tmp_path) -> None:
        ...
        charts=[
            {
                ...
                "render_refs": [{"kind": "render_svg", "path": "renders/chart_a.svg"}],
            },
            {
                "chart_id": "chart_b",
                "title": "B",
                ...
                "field_mappings": [{"target_field": "status", "source_field": "status"}],
                "render_refs": [{"kind": "render_svg", "path": "renders/chart_b.svg"}],
            },
        ],
    )
    save_chart_pack_bundle(
        ...
        renders={
            "chart_a": {"svg": "<svg>a</svg>"},
            "chart_b": {"svg": "<svg>b</svg>"},
        },
        root=root,
    )
    ...
    save_chart_pack_bundle(
        ...
        renders={"chart_a": {"svg": "<svg>a2</svg>"}},
        root=root,
    )
...
def test_chart_pack_store_saves_additive_bundle_artifact_json(tmp_path) -> None:
    root = tmp_path / "chart_packs"
    chart_pack = _sample_chart_pack()
    save_chart_pack_bundle(
        chart_pack,
    ...
    artifact_path = save_chart_pack_artifact_json(
        chart_pack.chart_pack_id,
        "quality_gate.json",
        {"overall_status": "pass", "handoff_ready": True},
        root=root,
    )

    assert artifact_path == chart_pack_artifact_path(chart_pack.chart_pack_id, "quality_gate.json", root)
    payload = json.loads(artifact_path.read_text(encoding="utf-8"))
    assert payload["overall_status"] == "pass"
    assert payload["handoff_ready"] is True
    assert load_chart_pack_artifact_json(chart_pack.chart_pack_id, "quality_gate.json", root)["overall_status"] == "pass"
