...
def test_paper_synthesis_bundle_route_audit_script_ignores_split_routes_and_generated_reports(tmp_path):
    workspace = tmp_path / "workspace"
    _write(
        workspace / "tests" / "test_bridge.py",
        'client.get("/api/paper-syntheses/papersynth_browser_bridge_001/markdown")\n',
    )
    _write(
        workspace / "build" / "lib" / "scripts" / "generated.py",
        'route = "/paper-syntheses/papersynth_generated"\n',
    )
    _write(
        workspace / "docs" / "reports" / "historical.md",
        "`GET /paper-syntheses/{synthesis_id}` was the compatibility route.\n",
    )
