...
export function getNotebookFromBundle(bundle: ArtifactBundle): NotebookArtifact {
  ...
  const claims =
    claimsetData && typeof claimsetData === "object" && Array.isArray((claimsetData as { claims?: unknown[] }).claims)
      ? (claimsetData as { claims: Array<{ claim_id?: string; claim_text?: string; confidence?: string }> }).claims
      : [];
  ...
  const parsedClaims = claims.slice(0, 20).map((claim, index) => {
    const confidence: "low" | "medium" | "high" =
      claim.confidence === "high" || claim.confidence === "medium" || claim.confidence === "low"
        ? claim.confidence
        : "medium";
    return {
      claim_id: claim.claim_id ?? `claim-${index + 1}`,
      text: claim.claim_text ?? "Claim text missing",
      confidence,
    };
  });
