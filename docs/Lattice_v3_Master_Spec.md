...
The current implementation is enforced by the runtime hardening PR through `_mark_paper_deepread_indexed` in `backend/services/job_runner.py` and targeted tests in `tests/test_pipeline_residual_hardening_contract.py`.
