...
| `scripts/backfill_operational_outputs.py` | Obsidian exports and/or job enqueue side effects | yes | yes | partial | delegated | yes | yellow | Applies only with selected action flags. `--export-missing` requires vault-backup confirmation, `--enqueue-claimset` requires enqueue confirmation, and apply actions create a SQLite backup first. Enqueued jobs remain queue side effects. |
