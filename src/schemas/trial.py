...
class TrialExtraction(BaseModel):
    ...
    def to_summary_block(self) -> str:
        ...
        if self.intervention.dose_value > 0:
            unit = (
                self.intervention.dose_unit.replace("_per_day", "/d")
                if self.intervention.dose_unit != "unknown"
                else ""
            )
        ...
        if self.ketone_confirmation.measured:
            metric = (
                f" ({self.ketone_confirmation.metric})"
                if self.ketone_confirmation.metric != "unknown"
                else ""
            )
